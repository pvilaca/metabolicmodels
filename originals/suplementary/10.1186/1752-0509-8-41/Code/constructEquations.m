function [equationStrings rxnDif]=constructEquations(model,rxns,useComps,sortRevRxns,sortMetNames)
% constructEquations
...
for i=1:numel(indexes)
    ...
    for j=1:numel(reactants)
        ...
        if useComps==true
            cc=model.metComps(reactants(j));
            if iscell(cc)
                eqn=[eqn plusString stoich model.metNames{reactants(j)} '[' model.compNames{find(ismember(model.comps,model.metComps(reactants(j))))} ']'];
            else
                eqn=[eqn plusString stoich model.metNames{reactants(j)} '[' model.compNames{model.metComps(reactants(j))} ']'];
            end
        else
    ...
    for j=1:numel(products)
        ...
        if useComps==true
            cc=model.metComps(products(j));
            if iscell(cc)
               eqn=[eqn plusString stoich model.metNames{products(j)} '[' model.compNames{find(ismember(model.comps,model.metComps(products(j))))} ']'];
            else
               eqn=[eqn plusString stoich model.metNames{products(j)} '[' model.compNames{find(ismember(model.comps,num2str(model.metComps(products(j)))))} ']'];            end
    ...
    rxnDif{i}=model.rxnNames(indexes(i));
